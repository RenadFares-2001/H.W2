...
dim(nobel)
```
represent one person who had get noble 
935-observations
26-variables 
...
nobel_living_science %>%
   ...
   geom_bar(stat = "identity", position = "dodge") + coord_
```
...
nobel_living_science <- nobel_living_science %>%
     mutate(
       born_country_us = if_else(born_country == "USA", "USA",  "Other"))
      nobel_living_science %>% ggplot(aes(x = country_us,y=born_country_us, fill =category)) 
     geom_bar(stat = "identity", position = "dodge", orientation ="horizontal")
```
Yes,because in buzzfeed we analyzing the data to search for more understanding the data and get information from it we go deep and deep and make a lot of flirtations to understanding the data
